...
function exclui(id) {
    ...
    if (confirmacao) {
        ...
        if (tbody) {
            ...
            RemoveTableFromLocalStogare(id)
            RemoveTableArray(id)
        }
    }
}

function RemoveTableFromLocalStogare(id) {
    let get_aryCliente = JSON.parse(localStorage.getItem("cliente"))

    get_aryCliente = get_aryCliente.filter(element => {
        return element.id_cliente != id
    })

    localStorage.setItem("cliente", JSON.stringify(get_aryCliente))
}

function RemoveTableArray(id) {
    ary = ary.filter(element => {
        return element.id_cliente != id
    })
}